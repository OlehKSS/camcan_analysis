...
from camcan.utils import (run_stacking_spoc, run_ridge, run_meg_spoc)
...
meg_data = [d for d in meg_data if d.get('subject') in common_subjects]
...
for key, data in data_ref.items():
    if 'Stack' in key:
        df_pred, arr_mae, arr_r2, train_sizes, train_scores, test_scores =\
            run_stacking_spoc(data, subjects_data, cv=CV, fbands=FREQ_BANDS)
    elif key == 'MEG':
        df_pred, arr_mae, arr_r2, train_sizes, train_scores, test_scores =\
            run_meg_spoc(data, subjects_data, cv=CV, fbands=FREQ_BANDS)
    else:
        df_pred, arr_mae, arr_r2, train_sizes, train_scores, test_scores =\
