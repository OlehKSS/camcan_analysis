...
CONNECT_DATA_TRUNC = '../../data/connectivity/connect_data_truncated.h5'
...
df_columns = ['MAE', 'STD', 'duration', 'connect', 'atlas']
df_columns.append([f'MAE{i}' for i in range(0, CV)])
n_rows = len(TIMESERIES_DURATIONS) * len(CONNECTIVITY_KINDS) *\
    len(ATLASES_DESCR)
regression_mae = pd.DataFrame(index=range(0, n_rows),
...
subjects_predictions = pd.DataFrame(subjects_data.age,
                                    index=subjects_data.index, dtype=float)
...
results_iterator = product(TIMESERIES_DURATIONS,
                           CONNECTIVITY_KINDS,
                           ATLASES_DESCR)
for i, (d, c, a) in enumerate(results_iterator):
    hdf5_key = f'ts_{d}_{c}_{a}'
    connect_data = pd.read_hdf(CONNECT_DATA_TRUNC,
                               key=hdf5_key)
