...
CAMCAN_TIMESERIES = '/storage/tompouce/okozynet/camcan/timeseries-truncated'
...
for dur in TIMESERIES_DURATIONS:
    ts_dir = os.path.join(CAMCAN_TIMESERIES, 'ts_%s' % dur)
    ...
    for atlas, atlas_descr in zip(ATLASES, ATLASES_DESCR):
            ...
            for ts, subject_id in zip(time_series, dataset.subject_id):
                path_subject = os.path.join(CAMCAN_TIMESERIES, ts_dir, subject_id, atlas_descr)
...
out_file = os.path.join(OUT_DIR, 'connect_data_truncated.h5')
...
for dur in TIMESERIES_DURATIONS:
    str_dur = 'ts_%s' % dur
    ts_dir = os.path.join(CAMCAN_TIMESERIES, str_dur)
